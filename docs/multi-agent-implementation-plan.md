...
- 已落地：Phase 1-6 的主体代码已在仓库中存在，`ai.routes.ts` 的多 Agent 切换与 `multi-agent.eval.test.ts` 也已落地。
- 部分落地：Weather Agent 仍使用 Mock 数据；Notification Agent 尚未接入真实短信/邮件通道；checkpointer 仍为 `MemorySaver`；单 Agent 路径仍保留。
- 未核验项：本次文档整理没有重新执行数据库迁移、全量测试和手动端到端验收，因此文中涉及“运行验证/验收”的条目只按当前仓库状态标记，不等同于本次已复跑。

## 关键决策与约束

### D1: 使用 createReactAgent（非 createAgent） [已采用]
...
手动测试 + eval 测试是本阶段的验收目标。当前仓库已包含 eval 测试文件，但是否全部通过仍应以实际执行结果为准。
...
### Phase 2: Task Agent [已落地]
10. [x] 编写 `task.tools.ts`：5 个 Tool 定义 + 共用模糊查找函数 findTaskByTitleOrId
11. [x] 编写 `task.agent.ts`：createTaskAgent 函数 + TASK_AGENT_PROMPT
12. [x] 编写 `task.tools.test.ts`：5 个正常 case + 7 个异常 case
13. [x] 编写 `task.agent.test.ts`：3 个 Graph 级集成测试
14. [ ] 运行测试验证 Phase 2：测试文件与命令已具备，但本次文档整理未复跑

### Phase 3: Weather Agent [已落地，当前仍为 Mock 数据]
15. [x] 编写 `weather.tools.ts`：1 个 Tool（get_weather）+ Mock 天气数据
16. [x] 编写 `weather.agent.ts`：createWeatherAgent 函数
17. [x] 编写 `weather.tools.test.ts`：1 个正常 case + 2 个异常 case
18. [x] 编写 `weather.agent.test.ts`：1 个 Graph 级集成测试
19. [ ] 运行测试验证 Phase 3：测试文件与命令已具备，但本次文档整理未复跑

### Phase 4: Calendar Agent [已落地]
20. [x] 编写 `calendar.tools.ts`：2 个 Tool（get_day_schedule、find_free_slots）+ 空闲区间算法
21. [x] 编写 `calendar.agent.ts`：createCalendarAgent 函数
22. [x] 编写 `calendar.tools.test.ts`：2 个正常 case + 3 个异常 case
23. [x] 编写 `calendar.agent.test.ts`：1 个 Graph 级集成测试
24. [ ] 运行测试验证 Phase 4：测试文件与命令已具备，但本次文档整理未复跑

### Phase 5: Notification Agent [部分落地]
25. [x] 修改 `schema.ts`：新增 reminderStatusEnum、reminderChannelEnum、reminders 表、remindersRelations
26. [ ] 运行数据库迁移：`pnpm -C packages/server db-push`，实际环境执行情况需单独确认
27. [x] 编写 `notification.tools.ts`：3 个 Tool + 提醒时间计算逻辑 + 提醒内容生成逻辑
28. [x] 编写 `notification.agent.ts`：createNotificationAgent 函数
29. [x] 编写 `notification.tools.test.ts`：3 个正常 case + 3 个异常 case
30. [x] 编写 `notification.agent.test.ts`：1 个 Graph 级集成测试
31. [ ] 运行测试验证 Phase 5：测试文件与命令已具备，但本次文档整理未复跑

### Phase 6: Supervisor 编排 [已落地]
32. [x] 编写 `supervisor.ts`：buildSupervisorGraph 函数 + SUPERVISOR_PROMPT
33. [x] 编写 `index.ts`：MultiAgentService 类 + chat() 方法 + extractPayloadFromMessages + saveMessages
34. [x] 编写 `supervisor.test.ts`：5 个路由测试 + 2 个错误处理测试
35. [ ] 运行测试验证 Phase 6：测试文件与命令已具备，但本次文档整理未复跑

### Phase 7: 接入与评测 [部分落地]
36. [x] 修改 `ai.routes.ts`：添加 multi-agent 切换逻辑
