...
- 时间支持两种模式：具体时间段（startTime + endTime）与模糊时间段（timeSegment）。
- timeSegment 取值：all_day / early_morning / morning / forenoon / noon / afternoon / evening。
- 中文含义与边界：全天 00:00–23:59；凌晨 00:00–05:59；早上 06:00–08:59；上午 09:00–11:59；中午 12:00–13:59；下午 14:00–17:59；晚上 18:00–23:59。
- 未提供具体时间段时使用 timeSegment；默认策略与当前日期相关：非今天默认 all_day，今天白天默认 all_day，今天晚上默认 evening。
- 冲突检测仅对具体时间段任务生效。
- 仅对“今天”生效的限制：不能选择已经过去的模糊时段；如果当前已到晚上，则不能再选择 all_day，也不能选择更早的时段。

**数据模型补充：**
- time_segment 枚举更新为：all_day | early_morning | morning | forenoon | noon | afternoon | evening
