...
- `ENABLE_MULTI_AGENT`（可选，严格等于 `true` 才启用多 Agent）

`ENABLE_MULTI_AGENT` 建议值：

- Cloudflare（尤其是 Free 场景）：`ENABLE_MULTI_AGENT=false`（默认单 Agent，优先稳定性）
- 本地 / 普通 Node.js：`ENABLE_MULTI_AGENT=true`（启用 LangGraph 多 Agent）
